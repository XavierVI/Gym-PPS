...
class ReplayBuffer(object):
    ...
    def push(self, observations_original, actions_original, rewards_original, next_observations_original, dones_original):
        ...
        # self.nentries_act, self.nentries_act2 = np.shape(actions)[0], np.shape(actions)[1]
        # self.nentries_rew, self.nentries_rew2 = np.shape(rewards)[0], np.shape(rewards)[1]
        # self.nentries_next_obs, self.nentries_next_obs2 = np.shape(next_observations)[0], np.shape(next_observations)[1]
        # self.nentries_done, self.nentries_done2 = np.shape(dones)[0], np.shape(dones)[1]

        # print("reward_dim1", self.nentries_done)
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        # index_rew = self.nentries_rew * inds[:, None] + np.arange(self.nentries_rew)
        # index_next_obs = self.nentries_next_obs * inds[:, None] + np.arange(self.nentries_next_obs)
        # index_done = self.nentries_done * inds[:, None] + np.arange(self.nentries_done)

        # index_obs=index_obs.reshape(-1)
        # index_act=index_act.reshape(-1)
        # index_rew=index_rew.reshape(-1)
        # index_next_obs=index_next_obs.reshape(-1)
        # index_done=index_done.reshape(-1)
        ...
        # if norm_rews:
        #     ret_rews = cast((self.rew_buffs[index_rew, :] -
