...
class IDDPG(object):
    ...
    def step(self, observations, obs_slices, explore=False):
        ...
        """
        return [
            self.agents[i].step(observations[:, obs_slices[i]].t(), explore=explore)
    ...
    def _select_action_for_agent(self, acs, agent_i, act_slices=None):
        ...
        if acs.dim() != 2:
            raise RuntimeError(f"Expected acs to be 2D [B, K], got shape {tuple(acs.shape)}")
    ...
    def update(
        ...
        if obs.dim() != 2:
            raise RuntimeError(f"Expected obs to be [B, obs_dim], got {tuple(obs.shape)}")
        if next_obs.dim() != 2:
            raise RuntimeError(f"Expected next_obs to be [B, obs_dim], got {tuple(next_obs.shape)}")

        # choose action for this agent robustly
        act_i = self._select_action_for_agent(acs, agent_i, act_slices=act_slices)
        ...
        target_q = rew_i + self.gamma * curr_agent.target_critic(trgt_vf_in) * (1.0 - done_i)
        ...
        if logger is not None:
            logger.add_scalars(
                ...
                {"vf_loss": float(vf_loss.item()), "pol_loss": float(pol_loss.item())},
                self.niter,
            )
    ...
    def init_from_env(
        ...
        for _ in range(nagents):
            agent_init_params.append(
                {"num_in_pol": obs_dim, "num_out_pol": act_dim, "num_in_critic": num_in_critic}
            )
