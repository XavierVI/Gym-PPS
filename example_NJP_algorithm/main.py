...
def _load_algo(config, env, start_stop_num):
    ...
    if config.alg.upper() == "DDPG":
        ...
        if DDPGCls is None:
            raise ImportError("Cannot find class IDDPG or DDPG in algorithms/ddpg.py")
        ...
        act_slices = [slice(0, act_dim_ctrl), slice(act_dim_ctrl, 2 * act_dim_ctrl)]
    ...
    raise ValueError(f"Unknown --alg {config.alg}. Choose from MADDPG or DDPG.")
...
def run(config):
    ...
    for ep_i in tqdm(range(0, config.n_episodes, config.n_rollout_threads)):
        ...
        for et_i in range(config.episode_length):
            ...
            torch_agent_actions = algo.step(torch_obs, start_stop_num, explore=True)
        ...
        for _ in range(config.n_updates_per_episode):
            ...
            for a_i in range(getattr(algo, "nagents", 2)):
                ...
                _algo_update(algo, obs_s, acs_s, rews_s, next_obs_s, dones_s, a_i, start_stop_num)
        ...
        algo.epsilon = max(config.min_epsilon, algo.epsilon - config.epsilon_decay)
...
if __name__ == "__main__":
    ...
    # NEW: pick algorithm globally
    parser.add_argument("--alg", default="MADDPG", type=str, choices=["MADDPG", "DDPG"])

    # Training updates per episode (your old code had fixed 50)
