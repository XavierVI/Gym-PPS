...
"""
Reference implementation of a MARL algorithm for the PPS environment,
...
def run(config):
    ...
    # <<< HERE: paste the force-to-device block >>>
    ...
    for ag in getattr(maddpg, "agents", []):
        for attr in ["policy", "actor", "critic"]:
            ...
            if m is not None and hasattr(m, "parameters"):
                try:
                    print("First agent", attr, "param device:", next(m.parameters()).device)
                    printed = True
                    break
    ...
    for ep_i in tqdm(range(0, config.n_episodes, config.n_rollout_threads)):
        ...
        for et_i in range(config.episode_length):
            # -------- render switch (headless-safe) --------
            # Only render if user enabled it AND interval matches.
        ...
        for _ in range(50):
            ...
            for a_i in range(maddpg.nagents):
                # buffer_total[0] for predators, buffer_total[1] for prey
                # NOTE: this matches the original code's assumption.
